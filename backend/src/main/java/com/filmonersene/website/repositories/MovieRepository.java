...
public interface MovieRepository extends JpaRepository<Movie, Long>{
	...
	int countByRecommendedBy_Id(Long userId);
...
*/
	...
	@Query(value = """
		    SELECT m.*, COUNT(ml.id) as like_count
		    FROM movies m
		    LEFT JOIN movie_likes ml ON m.id = ml.movie_id AND ml.liked = true
		    GROUP BY m.id
		    ORDER BY COUNT(ml.id) DESC, m.id ASC
		    """,
		    countQuery = "SELECT COUNT(*) FROM movies",
		    nativeQuery = true)
		Page<Movie> findMostLikedMoviesNative(Pageable pageable);
	...
	@Query(value = """
		    ...
		    """,
		    nativeQuery = true)
		List<Movie> findTop7MostLikedMovies();

	List<Movie> findTop20ByMovieNameContainingIgnoreCase(String movieName);
